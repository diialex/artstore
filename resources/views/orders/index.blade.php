...
@section('content')
    <div class="row m-4">
        ...
        @if(auth()->user()->roles->contains('id', 1))
            <a href="{{ route('orders.create') }}" class="btn btn-dark btn-sm rounded-pill px-4 tracking-wide">
                <i class="bi bi-plus-lg me-1"></i> @lang('messages.neworder')
            </a>
        @endif
    </div>
    ...
    @if($orders->isEmpty())
        <div class="text-center py-5">
            ...
            <h3 class="fw-light mt-4">@lang('messages.no_orders')</h3>
            <a href="{{ route('home') }}" class="btn btn-outline-dark rounded-pill px-5 mt-3 fw-bold">@lang('messages.store_go')</a>
        </div>
    @else
        <div class="row g-3">
                        ...
                        <div class="card-body p-4">
                            <div class="row align-items-center">
                                <div class="col-md-3 border-end-md">
                                    <span class="text-muted x-small text-uppercase fw-bold ls-1">@lang('messages.reference')</span>
                                    <h5 class="fw-bold mb-0">#{{ str_pad($order->id, 5, '0', STR_PAD_LEFT) }}</h5>
                                </div>

                                <div class="col-md-3 mt-3 mt-md-0">
                                    <span class="text-muted x-small text-uppercase fw-bold ls-1">@lang('messages.date')</span>
                                    <p class="mb-0 fw-medium">{{ $order->created_at->format('d M, Y') }}</p>
                                </div>

                                <div class="col-md-2 mt-3 mt-md-0">
                                    <span class="text-muted x-small text-uppercase fw-bold ls-1">@lang('messages.total')</span>
